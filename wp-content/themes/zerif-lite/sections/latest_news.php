			...
			echo '<div class="container">';
				...
				echo '<div id="carousel-homepage-latestnews" class="carousel slide" data-ride="carousel">';
					...
					echo '<div class="carousel-inner" role="listbox">';
						...
						$zerif_latest_loop = new WP_Query( array( 'post_type' => 'post', 'posts_per_page' => $zerif_total_posts, 'cat'=>'-3' , 'order' => 'DESC','ignore_sticky_posts' => true ) ); // 在条件中增加了条件'cat' => '-3'表示排除解决方案 

						$newSlideActive = '<div class="item active">';
						$newSlide 		= '<div class="item">';
						$newSlideClose 	= '<div class="clear"></div></div>';
						$i_latest_posts= 0; //query_posts('cat=-3');
						while ( $zerif_latest_loop->have_posts() ) : $zerif_latest_loop->the_post();

							$i_latest_posts++;
							//$is_category =  in_category('3');
							//if ( !wp_is_mobile() && !$is_category){  //&& in_category('3') 添加除去解决方案条件
							if ( !wp_is_mobile() ){
									if($i_latest_posts == 1){
										echo $newSlideActive;
									}
